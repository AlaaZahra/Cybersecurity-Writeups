...
The investigation started by confirming the dataset and reviewing the raw CloudTrail events:

```spl
index="aws_cloudtrail"
```

![Initial AWS CloudTrail dataset overview](images/00-cloudtrail-overview.png)

The dataset contained `4,032` events. The following query then counted the number of events generated by each AWS service:
...
![Chronological console-login sequence](images/02-console-login-sequence.png)
...
![Failed console logins grouped by identity and IP](images/03-failed-console-logins.png)
...
![Activity associated with the compromised identity](images/04-compromised-user-activity.png)
...
![First S3 enumeration activity](images/05-first-s3-enumeration.png)
...
![S3 GetObject events attributed to the compromised user](images/06-s3-getobject-access.png)
...
![GetObject result containing the DWG file and its bucket](images/06-s3-getobject-access.png)
...
![Query result identifying the affected S3 bucket](images/07-s3-public-access-query.png)

The matching event was expanded to verify the values submitted inside `requestParameters.publicAccessBlockConfiguration`:

![Expanded event showing all four public-access safeguards set to false](images/08-s3-public-access-details.png)
...
![New IAM user created by the compromised identity](images/09-iam-user-created.png)
...
![New IAM user added to the Admins group](images/10-iam-user-added-to-group.png)
